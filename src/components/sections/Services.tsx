...
function ServiceCard({ service, index }: { service: typeof services[0]; index: number }) {
  ...
  return (
    ...
    >
      <TooltipTrigger text={`Learn more about ${service.title}`}>
        <motion.div
          ref={cardRef}
          ...
          className="group relative h-full p-8 rounded-2xl bg-card border border-border hover:border-primary/50 transition-colors duration-500 shadow-lg hover:shadow-2xl"
        >
          ...
          {/* Icon */}
          <div className={`inline-flex p-4 rounded-2xl bg-${service.color}/10 mb-6`}>
          ...
          <div className="flex flex-wrap gap-2">
            {service.skills.map((skill) => (
              <span
                key={skill}
                className="px-3 py-1.5 text-xs font-mono rounded-xl bg-secondary text-secondary-foreground"
              >
                {skill}
              </span>
            ))}
          ...
          <div className="absolute top-0 right-0 w-24 h-24 overflow-hidden rounded-tr-2xl pointer-events-none">
            <div className={`absolute top-0 right-0 w-24 h-0.5 bg-gradient-to-l from-${service.color} to-transparent`} />
            <div className={`absolute top-0 right-0 w-0.5 h-24 bg-gradient-to-b from-${service.color} to-transparent`} />
          </div>
        </motion.div>
      </TooltipTrigger>
    </motion.div>
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        >
          {additionalSkills.map((skill) => (
            <TooltipTrigger key={skill.name} text={skill.name}>
              <motion.div
                whileHover={{ scale: 1.05, y: -2 }}
                className="flex items-center gap-3 px-6 py-3 rounded-2xl border border-border bg-card/50 backdrop-blur-sm shadow-sm hover:shadow-md transition-shadow"
              >
